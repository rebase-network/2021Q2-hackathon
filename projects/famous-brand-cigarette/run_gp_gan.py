...
def main():
    ...
    for idx in range(total_size):
        ...
        # load image
        obj = img_as_float(imread(test_list[idx][0]))[:, :, :3]
        #obj_content = np.sum(obj, axis=-1)
        #mask = np.zeros((800, 800)).astype(np.float32)
        #mask[obj_content > 0.] = 1.0
        #mask_image = Image.fromarray(np.uint8(mask * 255), 'L')
        #mask_image.save('/home/ruotongw/Workspace/GP-GAN/doge/mask_show.jpg')
        bg = img_as_float(imread(test_list[idx][1]))[:, :, :3]
        bg = bg[0:800, 0:800, :]
        mask = imread(test_list[idx][2], as_gray=True).astype(obj.dtype) / 255.0
        #mask = imread(test_list[idx][2], as_gray=True).astype(obj.dtype)
        #mask = np.zeros((800, 800))
        #mask[115:645][195:638] = 1.0
