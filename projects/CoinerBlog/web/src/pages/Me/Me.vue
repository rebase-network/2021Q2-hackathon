  ...
  <div class="me">
    <div class="profile-card card">
      ...
      <footer class="avatar-card-footer no-text-select">
        ...
        <a class="box-col able-to-active">
          <p>{{$store.state.follows.length}}</p>
          <p>关注</p>
        </a>
        <!-- <i class="separate-line"></i>
      <a class="box-col able-to-active">
    ...
    <div
      class="card-line-group rubberBand"
        ...
        >
          <div class="statistics-card">
            ...
            <p style="text-align: center">
              {{ statisticsData.start }} 至 {{ statisticsData.end }}
            </p>
            <h3>最近：</h3>
    ...
    <!-- <transition name="fade">
      <div class="stat-intro-wrapper" v-if="toShowStatIntro">
        <div class="intro-control-wrapper">
          <button
            class="btn btn-warning intro-confirm-btn"
            @click="hideStatIntro()"
          >
            知道了！
          </button>
...
export default {
  ...
  mounted: async function () {
    let haveIntroducedStat = localStorage.getItem("haveIntroducedStat")
      ? JSON.parse(localStorage.getItem("haveIntroducedStat"))
      : false;
    if (!haveIntroducedStat) {
      this.showStatIntro();
    }
    try {
      this.blogLength = await this.$store.state.web3.routerFunc.getPersonBlogIdsLength(
